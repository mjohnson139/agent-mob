...
Each project branch is self-contained. The `participants:` map in `PROJECT.yml` defines who is working on that project — there is no global team roster. Per-member phase artifacts are named `@{id}.md`.
